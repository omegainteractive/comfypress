...
class HelperTagTest < ActiveSupport::TestCase
  ...
  def test_initialize_tag
    assert tag = ComfyPress::Tag::Helper.initialize_tag(
  ...
  def test_initialize_tag_failure
    [
      ...
      assert_nil ComfyPress::Tag::Helper.initialize_tag(
  ...
  def test_blacklisted_methods
    ComfyPress::Tag::Helper::BLACKLIST.each do |method|
      tag = ComfyPress::Tag::Helper.initialize_tag(
  ...
  def test_whitelisted_methods
    ComfyPress.config.allowed_helpers = [:tester, :eval]
    ComfyPress.config.allowed_helpers.each do |method|
      tag = ComfyPress::Tag::Helper.initialize_tag(
        cms_pages(:default), "{{ cms:helper:#{method}:Rails.env }}"
      )
      assert_equal "<%= #{method}('Rails.env') %>", tag.content
